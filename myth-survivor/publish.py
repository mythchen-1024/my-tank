#!/usr/bin/env python3
"""
AgenTank 出击(raid)坦克专用发布工具。

本脚本专门发布 myth-survivor 出击坦克到 raid(出击)分支：
  构建 raid-src/*.js -> raid-tank-submit.js，再发布到 raid。

用法（在 myth-survivor 目录下，或用绝对路径）:
  python publish.py                      # 构建并发布 raid-tank-submit.js 到 raid
  python publish.py --no-build           # 跳过构建，直接发布现有产物
  python publish.py --file path/to.js    # 发布指定文件(覆盖默认产物)
  python publish.py --notes "调多发"     # 附带版本说明
  python publish.py -b multiplayer       # 同一份代码发到多人分支
  python publish.py --no-minify          # 关闭压缩，原样发布
  python publish.py --dry-run            # 只打印请求体，不实际发布
  python publish.py --status             # 查看坦克当前状态(各分支版本)
  python publish.py --matches            # 查看最近战斗记录

坦克档案在 SURVIVOR_PROFILE 定义，绑定 key/默认产物/默认分支/构建命令。
...
# 出击坦克档案：survivor(id 3083, key agtk_cce8...)，默认发到 raid(出击)分支。
# 产物 raid-tank-submit.js 由 raid-src/*.js 经 build.js 拼接生成。
SURVIVOR_PROFILE = {
    "key": "agtk_cce872653e5d5b90f76db3ac370a2d2809b6",
    "file": "raid-tank-submit.js",
    "branch": "raid",
...
def main():
    # 路径相对脚本目录，保证从任意 cwd 运行都正确。
    os.chdir(os.path.dirname(os.path.abspath(__file__)) or ".")
    load_dotenv()

    parser = argparse.ArgumentParser(description="AgenTank 出击(raid)坦克专用发布工具")
    parser.add_argument("--file", "-f", default=None, help="要发布的 JS 文件 (默认: raid-tank-submit.js 产物)")
    ...
    parser.add_argument("--no-build",               action="store_false", dest="build",  help="跳过构建，直接发布现有产物")
    ...
    print(f"[出击坦克 survivor]  分支: {branch}  文件: {file_path}")
    ...
    if args.status:
        ...
        # 仅当配置了 build_cmd、用户未显式指定 --file、且非 dry-run 时才自动构建。

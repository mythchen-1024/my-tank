...
// 守线预开炮(lead shot)：守株敌横穿我炮线时，按「子弹2格/帧、敌1格/帧」算提前量，
// 在敌恰好走进子弹时空交点的那一帧开火，打移动靶（敌不躲弹→命中率极高）。
// 命中判据：开火后第 j 帧子弹沿炮线覆盖标量 {2j-1,2j}，敌走到 E_j 沿炮线前方距离 s∈{2j-1,2j}。
// 匹配本身=强门控，不乱开火。max 帧对应子弹有效射程（2*j≤10→j≤5）。
var LEAD_MAX_FRAMES = 5;       // 预开炮预测的最大提前帧数（覆盖约 10 格射程）

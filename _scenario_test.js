...
console.log('场景V3: findGuardLineShot 对overload流(冷却中)同线可开火');
{
  ...
  // 注：me 未对准敌人，返回预瞄转向(非开火)，不受"平局不对枪"抑制影响。
...
// =========================================================
// 场景 SF：复刻 mat_2BcJYDQUoHt9pEWtq f104 让出决胜星
// 我[12,12]、星[15,13]、overload敌[15,4](cd>5 手里没双弹)、双方都4星(领先视角)。
// 我到星 BFS≈4-5，敌绕墙 BFS≈9 -> 我是抢星热门。期望朝星(向东)走，不向西巡逻。
// 防过头(SF2)：敌贴星比我近时 favorite=false，不强抢。
// =========================================================
console.log('场景SF: overload敌冷却中，领先时也要抢热门星(mat_2Bc f104)');
{
  MATCH_STATE = null;
  const map = emptyMap(19, 15);
  const me = makeMe([12, 12], 'left', { stars: 4, skill: { type: 'teleport', remainingCooldownFrames: 30 } });
  const enemy = {
    tank: { id: 'e', position: [15, 4], direction: 'up', crashed: false },
    bullet: null,
    skill: { type: 'overload', remainingCooldownFrames: 30 },
    status: {},
    stars: 4
  };
  const game = { map, star: [15, 13], frames: 104 };
  const step = chooseMoveCandidateScored(me, enemy, game, enemy.tank.position, getMatchState(game), []);
  // 向星的下一步应朝东(x增大)或朝南(y增大)逼近 [15,13]，绝不向西(x减小)
  check('SF1 领先时抢热门星(不向西巡逻)', step && step.step && step.step[0] >= 12 && manhattan(step.step, game.star) < manhattan(me.tank.position, game.star),
    'step=' + JSON.stringify(step && step.step) + ' kind=' + (step && step.kind));
}

console.log('场景SF2: 敌比我更近热门星时不强抢(防过头)');
{
  MATCH_STATE = null;
  const map = emptyMap(19, 15);
  // 我远(在[2,2])、敌贴星[15,12]、星[15,13]：敌 BFS=1 我 BFS≈24 -> favorite=false
  const me = makeMe([2, 2], 'right', { stars: 4, skill: { type: 'teleport', remainingCooldownFrames: 30 } });
  const enemy = {
    tank: { id: 'e', position: [15, 12], direction: 'down', crashed: false },
    bullet: null,
    skill: { type: 'overload', remainingCooldownFrames: 30 },
    status: {},
    stars: 4
  ...
  const game = { map, star: [15, 13], frames: 104 };
  const state = getMatchState(game);
  const ctx = buildScoringContext(me, enemy, game, state, [], null, enemy.tank.position);
  check('SF2 敌更近时 isStarFavorite=false', ctx.isStarFavorite === false, 'isStarFavorite=' + ctx.isStarFavorite);
}

// =========================================================
// 场景 GL：findGuardLineShot 对 overload 流的“握没握双弹”分界
// GL1：cd>5(手里没双弹)、安全环带 d=5、即将同线 -> 允许预转/开火(非 null)。
// GL2：握双弹(overloaded 或 cd<=1) -> 同线/预转均 null(保守不变)。
// =========================================================
console.log('场景GL: overload没双弹恢复守线预瞄，握双弹仍保守');
{
  MATCH_STATE = null;
  const map = emptyMap(19, 15);
  // d=5：我[5,7] 敌[10,7] 同行，视线清晰 -> lineDir 应可开火/对准
  const me = makeMe([5, 7], 'right', { stars: 1, skill: { type: 'teleport', remainingCooldownFrames: 0 } });
  const enemyNoDouble = {
    tank: { id: 'e', position: [10, 7], direction: 'left', crashed: false },
    bullet: null,
    skill: { type: 'overload', remainingCooldownFrames: 30 },
    status: {},
    stars: 0
  };
  const game = { map, star: null, frames: 70 };
  const shotNoDouble = findGuardLineShot(me, enemyNoDouble, enemyNoDouble.tank, [], game, enemyNoDouble.tank.position);
  // 注：me 领先1星(非平局)，避免与"平局不对枪"新策略耦合；平局抑制另由 TIE 场景覆盖。
  check('GL1 overload冷却中(没双弹) d=5同线 -> 守线开火(非null)', shotNoDouble !== null, 'shot=' + JSON.stringify(shotNoDouble));

  const enemyHolding = {
    tank: { id: 'e', position: [10, 7], direction: 'left', crashed: false },
    bullet: null,
    skill: { type: 'overload', remainingCooldownFrames: 0 }, // cd<=1 握双弹
    status: {},
    stars: 0
  };
  const shotHolding = findGuardLineShot(me, enemyHolding, enemyHolding.tank, [], game, enemyHolding.tank.position);
  check('GL2 overload握双弹(cd=0) 同线仍不对枪(null)', shotHolding === null, 'shot=' + JSON.stringify(shotHolding));

  const enemyOverloaded = {
    tank: { id: 'e', position: [10, 7], direction: 'left', crashed: false },
    bullet: null,
    skill: { type: 'overload', remainingCooldownFrames: 30 },
    status: { overloaded: true }, // 已过载下帧双弹
    stars: 0
  };
  const shotOverloaded = findGuardLineShot(me, enemyOverloaded, enemyOverloaded.tank, [], game, enemyOverloaded.tank.position);
  check('GL2 overload已过载 同线仍不对枪(null)', shotOverloaded === null, 'shot=' + JSON.stringify(shotOverloaded));
}

// =========================================================
// 场景 FH：双 teleport 抢星——对手走路就能到星时直传星点抢分(mat_FH69HGVihyPEJzCjS)
// 复刻 f51：星 [3,5]，我 [14,4](走路 ~12 格够不到)，敌 [2,3] teleport 就绪但走路距星仅 3 格。
// 旧逻辑：enemyTeleportReady 即走十字相邻 -> 传 [4,5] 没吃到星、补吃 [3,5] 贴敌死区被拦 -> 丢星+废传送。
// 期望：对手走路 d=3 就能直接走过去吃，不会瞬移狙我 -> 我直传星点 [3,5] 当帧吃到，抢速度赢这颗。
// =========================================================
console.log('场景FH: 双teleport抢星-对手走路近时直传星点(mat_FH69)');
{
  const map = emptyMap(19, 15);
  const star = [3, 5];
  const me = makeMe([14, 4], 'up', { skill: { type: 'teleport', remainingCooldownFrames: 0 } });
  // 敌 teleport 就绪，但走路距星 [3,5] 仅 |2-3|+|3-5|=3 格
  const enemyNear = { tank: { id: 'e', position: [2, 3], direction: 'up', crashed: false }, bullet: null, skill: { type: 'teleport', remainingCooldownFrames: 0 }, status: {} };
  const game = { map: map, star: star, frames: 51 };
  const rNear = findStarTeleport(me, enemyNear, enemyNear.tank, [], game);
  check('FH1 敌走路近(d=3)就能吃星 -> 直传星点抢分,不传旁边', rNear && samePos(rNear, star), 'r=' + JSON.stringify(rNear));

  // 对照 FH2：同一颗星,敌远在 [15,12](走路距星 ~19 格够不到),会被迫瞬移狙我 -> 仍走十字相邻避狙。
  // 我方也放远([16,10] 走路 ~18 格 >5)避免触发"走路够快不浪费传送"提前返回。
  const meFar = makeMe([16, 10], 'up', { skill: { type: 'teleport', remainingCooldownFrames: 0 } });
  const enemyFar = { tank: { id: 'e', position: [15, 12], direction: 'down', crashed: false }, bullet: null, skill: { type: 'teleport', remainingCooldownFrames: 0 }, status: {} };
  const rFar = findStarTeleport(meFar, enemyFar, enemyFar.tank, [], game);
  check('FH2 敌走路远(够不到)会瞬移狙星线 -> 仍传十字相邻避狙', rFar && manhattan(rFar, star) === 1, 'r=' + JSON.stringify(rFar));
}

// =========================================================
// =========================================================
// 场景 GWX：复刻 mat_GwxblYdS4ZSDXd0wX（输 小强 teleport流，55帧 crashed，星 2:2）
// 四个失误的精确几何复刻。
// =========================================================
console.log('场景GWX: mat_GwxblYdS 四失误复刻');
{
  const map = emptyMap(21, 19);

  // --- 失误2&3：贴脸星抢星不抖动 ---
  // GWX-A f32: me[8,12]朝left刚到,星[8,13]脚下(d=1),敌[15,13]远(d=6)朝left无实弹。
  // 期望:prime star 短意图 + 当帧朝星(转down或go down方向),不被防瞄改道走开。
  {
    const me = makeMe([8, 12], 'left', { stars: 1, skill: { type: 'teleport', remainingCooldownFrames: 8 } });
    const foe = { tank: { id: 'e', position: [15, 13], direction: 'left', crashed: false }, bullet: null, skill: { type: 'teleport', remainingCooldownFrames: 8 }, status: {}, stars: 1 };
    const game = { map: map, star: [8, 13], frames: 32 };
    MATCH_STATE = null;
    const st = getMatchState(game);
    onIdle(me, foe, game);
    const acted = me._actions.length > 0;
    const movedAway = me._actions.some(a => a[0] === 'go') && me.tank.direction === 'left';
    check('GWX-A f32 贴脸星:有动作且不朝left走开(不丢脚边星)', acted && !movedAway, JSON.stringify(me._actions));
    check('GWX-A f32 贴脸星:锁定 star 短意图', st.shortIntent && st.shortIntent.kind === 'star', JSON.stringify(st.shortIntent));
  }
  // GWX-B f33: me[8,12]朝down(已转好),星[8,13]脚下,敌[14,13]朝left无实弹,star短意图续跑。
  // 期望:go 吃星(不因 enemyAimsAt 星格改道)。
  {
    const me = makeMe([8, 12], 'down', { stars: 1, skill: { type: 'teleport', remainingCooldownFrames: 8 } });
    const foe = { tank: { id: 'e', position: [14, 13], direction: 'left', crashed: false }, bullet: null, skill: { type: 'teleport', remainingCooldownFrames: 8 }, status: {}, stars: 1 };
    const game = { map: map, star: [8, 13], frames: 33 };
    MATCH_STATE = null;
    const st = getMatchState(game);
    primeShortIntent(st, 'star', [8, 13], 32, 2);
    onIdle(me, foe, game);
    check('GWX-B f33 贴脸星朝向已对:go 吃星(不改道)', me._actions.length === 1 && me._actions[0][0] === 'go', JSON.stringify(me._actions));
  }
  // GWX-B2 防过头:敌有实弹在途时,贴脸星不强行直奔(交弹道逻辑),避免撞实弹。
  {
    const me = makeMe([8, 12], 'down', { stars: 1, skill: { type: 'teleport', remainingCooldownFrames: 8 } });
    // 敌实弹沿 y=13 朝 left 飞,此刻 [11,13](会扫到星格[8,13])
    const foe = { tank: { id: 'e', position: [14, 13], direction: 'left', crashed: false }, bullet: { position: [11, 13], direction: 'left' }, skill: { type: 'teleport', remainingCooldownFrames: 8 }, status: {}, stars: 1 };
    const game = { map: map, star: [8, 13], frames: 35 };
    MATCH_STATE = null;
    const st = getMatchState(game);
    primeShortIntent(st, 'star', [8, 13], 34, 2);
    onIdle(me, foe, game);
    // 不能直接 go 撞进 y=13 实弹弹道
    const wentIntoBullet = me._actions.some(a => a[0] === 'go') && me.tank.direction === 'down';
    check('GWX-B2 防过头:敌有实弹扫星行时不直奔撞弹', !wentIntoBullet, JSON.stringify(me._actions));
  }

  // --- 失误4：传送落点紧贴横扫子弹,不传"走向会接子弹"的陷阱落点 ---
  // GWX-C f51: 星[15,14],敌子弹沿 y=12 朝 right 此刻[3,12]。直传星点[15,14]本身安全(y=14),
  // 但落地朝 up 走会经 [15,13]->[15,12] 撞进 y=12 子弹 -> landingNearSweepingBullet 判危险。
  {
    const gmC = { map: map, star: null, frames: 51 };
    const bullet = { position: [3, 12], direction: 'right' };
    check('GWX-C landingNearSweepingBullet([15,14]) 紧贴 y=12 横扫弹 -> true',
      landingNearSweepingBullet([15, 14], [bullet], gmC) === true);
    // 防过头:子弹离落点 >=3 行则不算陷阱
    const farBullet = { position: [3, 9], direction: 'right' };
    check('GWX-C2 防过头:子弹离落点3行(y=9 vs y=14)不算扫掠陷阱 -> false',
      landingNearSweepingBullet([15, 14], [farBullet], gmC) === false);
    // 防过头:子弹反向飞(已飞过落点列,right且已在x=18右侧)不威胁
    const passedBullet = { position: [18, 12], direction: 'right' };
    check('GWX-C3 防过头:子弹已飞过落点列(x=18>15)不算威胁 -> false',
      landingNearSweepingBullet([15, 14], [passedBullet], gmC) === false);
  }

  // --- 失误1：被动跑分 teleport 敌,无星空窗期巡逻偏向中心 ---
  // GWX-D: 无星,敌teleport流在角落[2,2]无实弹没瞄我,我在[5,5]。巡逻目标应偏中心,不奔对侧远角。
  {
    const me = makeMe([5, 5], 'right', { stars: 1, skill: { type: 'teleport', remainingCooldownFrames: 8 } });
    const foe = { tank: { id: 'e', position: [2, 2], direction: 'left', crashed: false }, bullet: null, skill: { type: 'teleport', remainingCooldownFrames: 8 }, status: {}, stars: 1 };
    const game = { map: map, star: null, frames: 12 };
    MATCH_STATE = null;
    const st = getMatchState(game);
    check('GWX-D enemyIsPassiveRusher(teleport+无实弹+没瞄我) -> true',
      enemyIsPassiveRusher(foe, foe.tank, game, me.tank.position) === true);
    const cx = Math.floor(21 / 2), cy = Math.floor(19 / 2);
    const vt = virtualPatrolTarget(me, game, st, foe);
    // 巡逻目标应比"对侧远角[18,16]附近"更靠近中心
    const vtToCenter = vt ? Math.abs(vt[0] - cx) + Math.abs(vt[1] - cy) : 999;
    check('GWX-D 被动跑分敌:巡逻目标靠近中心(distToCenter<=6)', vt && vtToCenter <= 6, 'vt=' + JSON.stringify(vt) + ' dCenter=' + vtToCenter);
    // 防过头:敌有实弹时不算被动跑分
    const foeArmed = { tank: { id: 'e', position: [2, 2], direction: 'left', crashed: false }, bullet: { position: [3, 2], direction: 'right' }, skill: { type: 'teleport', remainingCooldownFrames: 8 }, status: {}, stars: 1 };
    check('GWX-D2 防过头:敌有实弹在途 -> 非被动跑分(正常避让)',
      enemyIsPassiveRusher(foeArmed, foeArmed.tank, game, me.tank.position) === false);
  }
}
...
// =========================================================
console.log('场景TIE: 平局不打躲不掉的对枪(mat_FXFkk0)');
{
  const map = emptyMap(20, 20);

  // TIE1: 复刻死帧——1:1 同列 d=2 双方都对准对射 -> 平局应抑制守线开火(返回非fire)
  // 死帧 DTrump 护盾已 expired(cd>0 未开)，是裸奔互射，必须抑制。
  const meT1 = makeMe([12, 9], 'up', { stars: 1, skill: { type: 'teleport', remainingCooldownFrames: 30 } });
  const foeT1 = { tank: { id: 'e', position: [12, 7], direction: 'down', crashed: false }, bullet: null, skill: { type: 'shield', remainingCooldownFrames: 8 }, status: {}, stars: 1 };
  const gT1 = { map: map, star: null, frames: 24 };
  const gsT1 = findGuardLineShot(meT1, foeT1, foeT1.tank, [], gT1, foeT1.tank.position);
  check('TIE1 平局同列对准d=2 -> 守线不开火(抑制互射)', !(gsT1 && gsT1.fire === true), 'g=' + JSON.stringify(gsT1));
  // d=2 敌已对准(enemyDuel=1)时侧移来不及(需2帧),lineDuel 给不出脱线格是正常的;
  // 关键是整体决策不能开火去锁死平局——onIdle 应走位/转向而非 fire。
  MATCH_STATE = null; meT1._actions.length = 0;
  onIdle(meT1, foeT1, gT1);
  check('TIE1 平局躲不掉也不开火(不锁死平局) -> onIdle无fire', !meT1._actions.some(a => a[0] === 'fire'), JSON.stringify(meT1._actions));

  // TIE2: 防过头——不平局(我领先1星)同样姿态 -> 照常守线开火(领先时该打就打)
  const meT2 = makeMe([12, 9], 'up', { stars: 2, skill: { type: 'teleport', remainingCooldownFrames: 30 } });
  const foeT2 = { tank: { id: 'e', position: [12, 7], direction: 'down', crashed: false }, bullet: null, skill: { type: 'shield', remainingCooldownFrames: 8 }, status: {}, stars: 1 };
  const gsT2 = findGuardLineShot(meT2, foeT2, foeT2.tank, [], { map: map, star: null, frames: 24 }, foeT2.tank.position);
  check('TIE2 领先1星同姿态 -> 守线照常开火(防过头)', gsT2 && gsT2.fire === true, 'g=' + JSON.stringify(gsT2));

  // TIE3: 严格先手放行——平局但敌背对我(需转身2帧才能还击)，我对准 d=2(2帧到) -> myDuel<enemyDuel -> 可打
  const meT3 = makeMe([6, 9], 'right', { stars: 1, skill: { type: 'teleport', remainingCooldownFrames: 30 } });
  const foeT3 = { tank: { id: 'e', position: [8, 9], direction: 'right', crashed: false }, bullet: null, skill: { type: 'teleport', remainingCooldownFrames: 5 }, status: {}, stars: 1 };
  const gsT3 = findGuardLineShot(meT3, foeT3, foeT3.tank, [], { map: map, star: null, frames: 40 }, foeT3.tank.position);
  check('TIE3 平局但严格先手(敌背对我) -> 干净击杀放行开火', gsT3 && gsT3.fire === true, 'g=' + JSON.stringify(gsT3));

  // TIE4: 平局未对准 -> 返回预瞄转向(不射子弹,不会互射) -> 不抑制
  const meT4 = makeMe([12, 9], 'right', { stars: 1, skill: { type: 'teleport', remainingCooldownFrames: 30 } });
  const foeT4 = { tank: { id: 'e', position: [12, 7], direction: 'left', crashed: false }, bullet: null, skill: { type: 'teleport', remainingCooldownFrames: 5 }, status: {}, stars: 1 };
  const gsT4 = findGuardLineShot(meT4, foeT4, foeT4.tank, [], { map: map, star: null, frames: 40 }, foeT4.tank.position);
  check('TIE4 平局未对准 -> 预瞄转向保留(非null,不开火)', gsT4 && gsT4.dir && !gsT4.fire, 'g=' + JSON.stringify(gsT4));

  // TIE5: 平局但敌可用护盾(cd=0就绪) -> 我这发被盾吃,非互射换命 -> 不抑制,照常守线
  // 用 d=4 + 上下有脱线空格,使 shieldDuelSafe 成立(开火后能横移),隔离"护盾就绪"这一判别点。
  const meT5 = makeMe([6, 9], 'right', { stars: 1, skill: { type: 'teleport', remainingCooldownFrames: 30 } });
  const foeT5 = { tank: { id: 'e', position: [10, 9], direction: 'left', crashed: false }, bullet: null, skill: { type: 'shield', remainingCooldownFrames: 0 }, status: {}, stars: 1 };
  const gsT5 = findGuardLineShot(meT5, foeT5, foeT5.tank, [], { map: map, star: null, frames: 40 }, foeT5.tank.position);
  check('TIE5 平局但敌护盾就绪 -> 不算互射,守线放行(非抑制)', gsT5 !== null, 'g=' + JSON.stringify(gsT5));

  // TIE6: 收窄关键——平局+同帧,但我"能侧移躲掉" -> 不抑制开火(交给line-duel去躲,不锁死成纯逃绕圈)。
  // me[6,9]朝down(垂直于敌我连线x轴),敌[10,9]朝left对准我,d=4。
  // myDuel=turn(down->right=1)+ceil(4/2=2)=3; enemyDuel=turn(left->left=0)+2=2 -> 我更慢(myDuel>enemyDuel)
  // -> 收窄后"我更慢"本就交躲避分支,不靠平局抑制额外压开火。验证 shouldSuppressTiedDuel=false。
  const meT6 = makeMe([6, 9], 'down', { stars: 1, skill: { type: 'teleport', remainingCooldownFrames: 30 } });
  const foeT6 = { tank: { id: 'e', position: [10, 9], direction: 'left', crashed: false }, bullet: null, skill: { type: 'teleport', remainingCooldownFrames: 5 }, status: {}, stars: 1 };
  check('TIE6 平局但我更慢(非同帧) -> 不抑制(交躲避分支,不绕圈跑路)',
    shouldSuppressTiedDuel(meT6, foeT6, foeT6.tank, { map: map, star: null, frames: 40 }, foeT6.tank.position, []) === false);

  // TIE7: 收窄关键——平局+同帧+但能1帧侧移脱线 -> 不抑制(交 line-duel-dodge 去躲,而非压死开火绕圈)。
  // me[6,9]朝up(垂直于敌我连线x轴,可本帧直接 go 到[6,8]离线),敌[10,9]朝up(需转左1帧对准我),同行d=4。
  // myDuel=turn(up->right=1)+ceil(4/2=2)=3; enemyDuel=turn(up->left=1)+2=3 -> 同帧;
  // 但我侧朝 up 即可1帧脱线 -> lineDuelEscapeExists=true -> 抑制应为 false(能躲就别压开火)。
  const meT7 = makeMe([6, 9], 'up', { stars: 1, skill: { type: 'teleport', remainingCooldownFrames: 30 } });
  const foeT7 = { tank: { id: 'e', position: [10, 9], direction: 'up', crashed: false }, bullet: null, skill: { type: 'teleport', remainingCooldownFrames: 5 }, status: {}, stars: 1 };
  check('TIE7 平局+同帧+能1帧脱线 -> 不抑制(让躲避周旋,不绕圈)',
    shouldSuppressTiedDuel(meT7, foeT7, foeT7.tank, { map: map, star: null, frames: 40 }, foeT7.tank.position, []) === false);
}
